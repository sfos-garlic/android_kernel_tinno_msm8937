...
static int AllocateMemory(void)
{
	...
	g_pTmpBuff = kmalloc(1024*80, GFP_ATOMIC);
	if(NULL == g_pTmpBuff) {
		pr_err("%s: fts: malloc TmpBuff failed\n", __func__);
		return -1;
	}
